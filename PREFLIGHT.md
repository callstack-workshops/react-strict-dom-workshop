...
- **First `rock run:ios` crashes during `pod install` with a Ruby encoding error** (`Unicode Normalization not appropriate for ASCII-8BIT` / `Encoding::CompatibilityError`) — your shell has a non-UTF-8 locale. Run `export LANG=en_US.UTF-8` (add it to your `~/.zshrc`), open a new terminal, and retry. A standard Terminal or iTerm session already sets this, so most machines never see it.
